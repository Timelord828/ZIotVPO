...
void ServiceManager::uninstall()
{
	...
	DeleteFile(L"Scanners.lep");
	DeleteFile(L"Monitors.lep");
	DeleteFile(L"Threats.lep");
...
void ServiceManager::createRegistryRecord()
{
	...
	Ret =
		RegCreateKeyEx(
			HKEY_LOCAL_MACHINE,
			TEXT("SOFTWARE\\Lavrukhina Anti-Virus"),
			0,
			NULL,
...
{
	HKEY hKey;
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Lavrukhina Anti-Virus"),
		0, KEY_READ | KEY_WOW64_64KEY, &hKey) == ERROR_SUCCESS)
	{
		RegCloseKey(hKey);

		RegDeleteTree(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Lavrukhina Anti-Virus"));
	}
}
