    ...
    : QMainWindow(parent)
{
	...
	connectToServer();
	threats = std::make_unique<ThreatList>(u"Threats.lep");
	loadMonitors();
	loadScanners();
}
...
{
	ui.reportTextEdit->setText(QString::fromUtf16(u"Выполняется сканирование..."));
...
void Client::scanRequest()
{
	...
	for (uint64_t i = 0; i < fileCount; i++)
	{
		...
		{
			QString report = QString::fromUtf16(path.c_str()) + QString::fromUtf16(u" не является угрозой");
			reportOutput(report);
		}
...
{
	std::u16string filePath = u"Monitors.lep";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;

	std::u16string header = reader.readU16String();
	if (header != u"Lavrukhina")
	{
		reader.close();
		return;
	}
...
{
	std::u16string filePath = u"Scanners.lep";
